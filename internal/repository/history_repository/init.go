...
const queryGetRecentActivities = `
	SELECT * FROM (
		SELECT
			mh.user_id,
			'manga' AS media_type,
...
var (
	...
	stmtGetRecentActivities *sqlx.NamedStmt
)
...
func Initialize() {
	...
	stmtGetRecentActivities, err = datastore.Get().Db.PrepareNamed(queryGetRecentActivities)
	if err != nil {
		logrus.Fatal(err)
	}
}
